...
class StoreUnitController extends Controller
{
        ...
        if ( $request->type != null)
            {
                if ($request->search != null and $request->type != null) // mamy rodzaj + wartość
                    {
                        $storeunits = DB::table('v_store_units')->where($request->type,'LIKE','%'.$request->search.'%')->orderby('updated_at','desc')->paginate(15);
                    }
                else
                    {
                        // szukamy pustych
            ...
            {
                $storeunits = DB::table('v_store_units')->orderby('updated_at','desc')->paginate(15);
            }
    ...
    public function store(Request $request)
    {
        ...
        for ($i = 1; $i <= $number; $i++) {
            ...
            $firm = '1234567';   // <- tutaj damy dane z konfiguracji magazynu - firmy
