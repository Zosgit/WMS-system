@extends('layouts.app')
@section('title') {{ 'Szczegóły dostawy' }} @endsection
@section('content')
...
<div class="container">
    <div class="card mb-4">
        <div class="card-header">
            Dostawa nr: <b>{{ $shipment->ship_nr}}</b>

        @if ($shipmentdetails->total() > 0)
            <form method="post" action="{{ route('shipmentdetail.send',['id' => $shipment->id]) }}">
                @csrf
                <button class="btn btn-success float-end px-4">Kontroluj</button>
            </form>
        @endif
      ...
      <div class="card-body">
        ...
        class="btn btn-primary float-start px-4 mb-4 btn-sm">Dodaj pozycję</a>
