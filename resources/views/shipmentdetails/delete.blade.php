    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLongTitle">{{ $shipmentdetail->product->code }}</h5>
            <button type="button" class="btn-close" data-coreui-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">Czy na pewno chcesz usunąć?
                <form id="delete-data" action="{{ route('shipmentdetail.destroy'['shipment' => $shipment, 'shipmentdetail' => $shipmentdetail]) }}" method="POST" class="d-none">
                    @method('Delete')
                    @csrf
