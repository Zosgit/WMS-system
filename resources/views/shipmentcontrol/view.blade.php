@extends('layouts.app')
@section('title') {{ 'Kontrola' }} @endsection
@section('content')
<a class="big" href="{{ route('control.index')}}"><svg class="icon">
    <use xlink:href="{{ asset('icons/coreui.svg#cil-arrow-left') }}"></use>
  </svg>&nbsp;Lista kontroli dostaw</a></br></br>
<div class="container mb-4">
    <div class="card">
      <div class="card-header"><div>Dostawa nr: <strong>{{ $shipment->ship_nr}}</strong>
    </div>

        @if ($full_control == 0 and $shipment->status_id == 403)
            <form method="post" action="{{ route('control.close',['id' => $shipment->id]) }}">
                @csrf
                <button class="btn btn-success float-end px-4">Zakończ kontrolę</button>
            </form>
        @endif
